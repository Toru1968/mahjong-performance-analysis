...
def analyze_player_advantage(all_game_results):
    """
    特定プレイヤーの偏り・優位性を検出。
    ...
    for game in all_game_results:
        flat.extend(game if isinstance(game, list) else [game])
    df = pd.DataFrame(flat)

    # 2. dict 型から数値を抽出
    df['runs_anomaly']   = df['p_runs']      < 0.05
    df['markov_anomaly']= df['markov'].apply(lambda d: d.get('p_value',1) < 0.05 if isinstance(d, dict) else False)
    df['comp_anomaly']   = df['p_empirical'] < 0.05

    # 3. プレイヤーごと要約
    grp = df.groupby('name').agg(
        n_games             = ('index', 'count'),
        rate_runs_anomaly   = ('runs_anomaly', 'mean'),
        rate_markov_anomaly = ('markov_anomaly','mean'),
        rate_comp_anomaly   = ('comp_anomaly','mean'),
        mean_cramers_v      = ('cramers_v','mean')
    ).reset_index()

    # 4. 全体分布との比較検定
    #    例: 有意率(rate_runs_anomaly) の KS 検定
    all_rates = grp['rate_runs_anomaly'].values
    def compare_rate(col):
        pvals = []
        for rate in grp[col]:
            stat,p = ks_2samp([rate],[*grp[col]])
            pvals.append(p)
        return pvals
   
    grp['ks_p_runs']   = compare_rate('rate_runs_anomaly')
    grp['ks_p_markov']= compare_rate('rate_markov_anomaly')
    grp['ks_p_comp']  = compare_rate('rate_comp_anomaly')

    # 5. プレイヤー×有意率のカイ二乗検定（2×2 テーブル例）
    chi2_p = []
    for seat, sub in df.groupby('name'):
        table = pd.crosstab(df['name']==seat, df['runs_anomaly'])
        chi2, p, *_ = chi2_contingency(table, correction=False)
        chi2_p.append(p)
    grp['chi2_p_runs'] = chi2_p

    return grp

def detect_advantage_for_game(game_results: List[Dict], alpha: float = 0.05):
    """
    1局分の分析結果 (analyze_hand_drawn_tile の戻り値 = プレイヤー辞書のリスト) から
    各席の偏りスコアを計算し、最も偏りの強い席を返す。
    スコア = runs_anomaly + markov_anomaly + comp_anomaly
    """
    scores = []
    for res in game_results:
        # 各検定を flag 化
        runs_fl  = int(res["p_runs"]      < alpha)
        markov_fl= int(res["markov"].get("p_value", 1.0) < alpha)
        comp_fl  = int(res["p_empirical"] < alpha)
        score = runs_fl + markov_fl + comp_fl
        scores.append({
            "name":   res["name"],
            "seat":   res["seat"],
            "score":  score,
            "p_runs": res["p_runs"],
            "p_markov": res["markov"].get("p_value", None),
            "p_comp": res["p_empirical"]
        })
    # 偏りスコア降順ソート
...
def save_csv(analyzed_results):
    ...
    for col in all_cols:
        if col not in df.columns:
           ...
           df[col] = 0 if col in ['index','p_runs','p_empirical','p_value',
                                  ...
                                  'tsumogiri_count','points_gained'] else ''
    ...
    # 指定した列にのみ置換を適用
    df[cols_to_replace] = df[cols_to_replace].replace(-1, '')
    # --- ★★★ 変更部分ここまで ★★★ ---
...
if __name__ == '__main__':
     ...
     if wall_list == "":
        ...
        for i in tqdm(range(num_iterations), desc="Analyzing Random Walls"):
         ...
         # ゲーム単位の偏りスコアを検出してレポート用リストに保存
         game_adv_list = []
         for game_idx, game_res in enumerate(all_results):
             adv = detect_advantage_for_game(game_res, alpha=0.05)
             for seat_info in adv:
                 game_adv_list.append({
                     "game_index": game_idx,
                     "name":       seat_info["name"],
                     "seat":       seat_info["seat"],
                     "bias_score": seat_info["score"],
                     "p_runs":     seat_info["p_runs"],
                     "p_markov":   seat_info["p_markov"],
                     "p_comp":     seat_info["p_comp"]
                 })

         # CSV で書き出し
         adv_df = pd.DataFrame(game_adv_list)
         adv_df.to_csv('game_by_game_advantage.csv', index=False, encoding='utf-8-sig')
         print("▶ game_by_game_advantage.csv に各局の優位性を出力しました。")

         generate_summary_report(all_results)

         save_csv(all_results)
         # ── プレイヤー優位性解析とCSV出力 ──
         adv_df = analyze_player_advantage(all_results)
         adv_df.to_csv('player_advantage_report.csv', index=False, encoding='utf-8-sig')
         print("▶ player_advantage_report.csv を出力しました。")